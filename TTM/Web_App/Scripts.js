  ...
  roles: [
    {
      ...
      textTransform: "none",
      variationOverrides: [{ index: 0, fontWeightAlias: "ExtraBold" }],
    },
    {
      name: "Body",
      shortName: "bd",
...
function computeScale(scale, base) {
  ...
  if (method === "Modular") {
    ...
    const mn = parseFloat(minSize) || seedSize * 0.5,
      mx = parseFloat(maxSize) || seedSize * 4;
...
function buildScaleSidebar() {
  ...
  el.innerHTML =
    `<h3 class="text-[var(--text-muted)] text-[11px] font-bold tracking-[1.2px] mb-2">TYPE SCALE</h3>` +
    steps
      .map(
        (s) => `
      <div class="flex items-center gap-3 py-2 border-b border-[var(--border)]">
        ...
        <span style="font-family:'${esc(ff)}';font-size:${Math.min(s.px, 28)}px;line-height:1;overflow:hidden;white-space:nowrap;max-width:130px;" class="font-semibold">quick brown fox jumps over the lazy dog</span>
    ...
    .map((role) => {
      ...
      ${vars
        ...
        <div class="py-2 border-b border-[var(--border)]">
          <div style="font-family:'${esc(v.fontFamily)}';font-size:${Math.min(v.scaleStep.px, 32)}px;line-height:${lhToCss(v.lineHeight)};letter-spacing:${lsToCss(v.letterSpacing)};font-weight:${v.fontWeight};overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">The quick brown fox jumps over the lazy dog</div>
    ...
    jsonEl.textContent = JSON.stringify(
      {
        ...
        roles: result.roleStyles.reduce((acc, s) => {
          ...
          if (g) g.variations.push(v);
          else acc.push({ role: s.roleName, variations: [v] });
          return acc;
        }, []),
        config: appState,
      },
...
function wireEvents() {
  ...
  wire("ws-base", (v) => {
    appState.baseFontSize = parseFloat(v) || 16;
  });
  wire("ws-method", (v) => {
    appState.scale.method = v;
  });
  wire("ws-seed", (v) => {
    appState.scale.seedSize = parseFloat(v) || 16;
  });
  wire("ws-steps", (v) => {
    appState.scale.steps = Math.max(1, parseInt(v) || 1);
  });
  wire("ws-ratio", (v) => {
    appState.scale.ratio = parseFloat(v) || 1.25;
  });
  wire("ws-naming", (v) => {
    appState.scale.namingScheme = v;
  });
}
